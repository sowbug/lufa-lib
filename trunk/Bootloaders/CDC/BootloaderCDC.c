...
/** \file
 *
 *  Main source file for the CDC class bootloader. This file contains
 *  the complete bootloader logic.
 */

#define  INCLUDE_FROM_BOOTLOADERCDC_C
#include "BootloaderCDC.h"

/** Contains the current baud rate and other settings of the first
 *  virtual serial port. This must be retained as some operating
 *  systems will not open the port unless the settings can be set
 *  successfully.
 */
...
/** Current address counter. This stores the current address of the
 *  FLASH or EEPROM as set by the host, and is used when reading or
 *  writing to the AVRs memory (either FLASH or EEPROM depending on
 *  the issued command.)
 */
static uint32_t CurrAddress;

/** Flag to indicate if the bootloader should be running, or should
 *  exit and allow the application code to run via a watchdog
 *  reset. When cleared the bootloader will exit, starting the
 *  watchdog and entering an infinite loop until the AVR restarts and
 ...
 *  required by the bootloader, then continuously runs the bootloader
 *  processing routine until instructed to soft-exit, or hard-reset
 *  via the watchdog to start the loaded application code.
 */
int main(void)
{
  char ch = MCUSR;
